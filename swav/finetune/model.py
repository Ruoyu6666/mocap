...
class SwAVSkeletonModel(nn.Module):
    ...
    mode:
      ...
      "finetune_last_n" - freeze all encoder params except the last `unfreeze_n` transformer blocks, given via `encoder_blocks_attr` 
                         (a middle ground: lets high-level features adapt while keeping most of the pretrained representation fixed)
    """
    def __init__(self, encoder: nn.Module, embed_dim: int, num_prototypes: int = 60,
                 mode: str = "finetune", unfreeze_n: int = 2, encoder_blocks_attr: str = "blocks", 
        ...
        # projection_head is always trainable. Pass a ProjectionHead(embed_dim, ...) to use it; 
        # prototypes then operate on its output dim, NOT embed_dim.
        ...
        if mode == "freeze":
            ...
            assert blocks is not None, (
                f"encoder has no attribute '{encoder_blocks_attr}' — pass the correct"
                f"attribute name holding your transformer block list via encoder_blocks_attr")
            # unfrozen blocks train normally via model.train()
            # keep the rest of the encoder (embeddings, frozen blocks) in eval mode
    ...
    def forward(self, clip: torch.Tensor, idx_a: torch.Tensor, idx_b: torch.Tensor):
        """
        Clip-input path: 
        matches an encoder that maps (B, T, J, C) -> (B, T, D) — i.e. one embedding PER FRAME, T of them, 
        from a single forward pass over the whole clip. This is the correct path for your encoder
