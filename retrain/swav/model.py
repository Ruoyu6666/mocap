...
class SwAVSkeletonModel(nn.Module):
    """
    mode:
      "finetune"        - encoder is fully trainable
      "freeze"          - encoder weights fixed, only the prototype layer (and thus the clustering of the existing
                           representation) is learned
      "finetune_last_n" - freeze all encoder params except the last `unfreeze_n` transformer blocks / layers, given
                           via `encoder_blocks_attr` (a middle ground: lets high-level features adapt while keeping most of
                           the pretrained representation fixed)
    """
    def __init__(self, encoder: nn.Module,
        ...
        # projection_head is always trainable, independent of `mode` — see ProjectionHead docstring. 
        # Pass a ProjectionHead(embed_dim, ...) to use it; prototypes then operate on its output dim, not embed_dim.
        ...
        if mode == "freeze":
            ...
            assert blocks is not None, (
                f"encoder has no attribute '{encoder_blocks_attr}' — pass the "
                f"correct attribute name holding your transformer block list "
                f"via encoder_blocks_attr"
            )
            for block in blocks[-unfreeze_n:]:
                for p in block.parameters():
                    p.requires_grad = True
            # keep the rest of the encoder (embeddings, frozen blocks) in
            # eval mode; unfrozen blocks train normally via model.train()
 
    def train(self, mode: bool = True):
        # override so encoder stays in eval() when frozen, even if someone
        # calls model.train() on the whole wrapper (e.g. at each epoch start)
        super().train(mode)
        if self.mode == "freeze":
            self.encoder.eval()
    ...
    def forward(self, x: torch.Tensor):
        """
        Single-frame-input path: x is (B, J, C) -> one embedding per sample.
        Only correct if self.encoder itself maps (B, J, C) -> (B, D) directly
        (i.e. NOT your clip-based encoder). Kept for the single-frame variant
        (train_one_epoch / sample_two_temporal_views). If your encoder maps
        (B, T, J, C) -> (B, T, D), use forward_clip() below instead — do not
        call this method with clip input.
        """
        if self.mode == "freeze":
            with torch.no_grad():
    ...
    def forward_clip(self, clip: torch.Tensor, idx_a: torch.Tensor, idx_b: torch.Tensor):
        ...
        — i.e. one embedding PER FRAME, T of them, from a single forward pass
        over the whole clip. This is the correct path for your encoder.
 
        clip:  (B, T, J, C)
        idx_a, idx_b: (B,) time indices (per-sample) selecting which of the
                      T frame embeddings to use as the two SwAV views.
 
        Returns: z_a, z_b (B, D) raw frame embeddings, p_a, p_b (B, D_head)
        projected+normalized embeddings, scores_a, scores_b (B, K) prototype
        logits for each view.
        """
        if self.mode == "freeze":
            with torch.no_grad():
    ...
    def embed_clip(self, clip: torch.Tensor):
        """
        Convenience: run the encoder (+ projection head) over a whole clip
        and return per-frame outputs for ALL T frames at once — useful when
        you want embeddings for every frame, not just two sampled views
        (e.g. at inference/representation-extraction time).
...
def sample_two_time_indices(center_idx: torch.Tensor, T: int, max_shift: int = 2, min_sep: int = 1):
    """
    Same sampling logic as sample_two_temporal_views, but returns clamped
    time indices only (no gather) — used when you already have per-frame
    embeddings for a whole clip (B, T, D) from a single encoder forward pass,
    and just need to pick two time steps out of it.
    """
