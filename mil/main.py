...
def main():
    ...
    parser.add_argument('--if_extract_feature', default=False, type=str2bool, help='if extract feature')
    ...
    if args.dataset in ['JapaneseVowels','SpokenArabicDigits','CharacterTrajectories','InsectWingbeat']:
    ...
    elif args.dataset in ["mabe_mice", "mocap","sdannce" ]:
        # load embedding
        if args.dataset == "mabe_mice":
            ...
            label_tr = []
            label_te = []
            for mouse_name in fmr1_fold_1["train"]:
                gen = data[mouse_name]["ratgen"]
                gen = [str(i) for i in gen]
                label_tr = label_tr + gen
            for mouse_name in fmr1_fold_1["valid"]:
                gen = data[mouse_name]["ratgen"]
                gen = [str(i) for i in gen]
                label_te = label_te + gen
            mapping = {s: i for i, s in enumerate(set(label_tr))}
