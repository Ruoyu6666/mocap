...
class InceptionModule(nn.Module):
    ...
    def __init__(self, in_channels: int,  out_channels: int = 32, bottleneck_channels: int = 32,
                padding_mode: str = "replicate",) -> None:
        ...
        for kernel_size in [10, 20, 30]:
            self.conv_layers.append(
                nn.Conv1d(in_channels=bottleneck_channels,
...
class InceptionBlock(nn.Module):
    ...
    def __init__(self,  in_channels: int, out_channels: int = 32, bottleneck_channels: int = 32, 
                padding_mode: str = "replicate", n_modules: int = 3,) -> None:
        super().__init__()
        # Create Inception modules that are run sequentially
        inception_modules = []
        for i in range(n_modules):
            inception_modules.append(
                InceptionModule(in_channels=in_channels if i == 0 else out_channels * 4,
                                out_channels=out_channels,
...
class InceptionTimeFeatureExtractor(nn.Module):
    ...
    def __init__(self, n_in_channels: int, out_channels: int = 32,
