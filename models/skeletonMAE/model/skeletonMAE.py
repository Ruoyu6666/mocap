...
from .layers import SkeleEmbed, Block, trunc_normal_
...
class SkeletonMAE(nn.Module):
    def __init__(self, dim_in=3, dim_feat=256, decoder_dim_feat=256, depth=5, decoder_depth=5, 
        ...
        #self.decoder_pred = nn.Linear(decoder_dim_feat, patch_size * dim_in, bias=True)
        #self.frame_upsampler = TemporalUpsampler(decoder_dim_feat, t_patch_size) if t_patch_size > 1 else nn.Identity()
    ...
    def random_masking(self, x, frame_mask_ratio=0.5, seg_len=5, joint_mask_ratio=0.5):
        N, L, D = x.shape
        ...
        # 1. SEGMENT-LEVEL frame masking: place a few-`seg_len` segments per sample until roughly frame_mask_ratio of TP are masked.
        num_frames_mask_target = max(int(TP * frame_mask_ratio), 1)
        ...
        for b in range(N):
            for s in range(num_segments):
                ...
                this_seg_len = min(seg_len, block_size)  # clamp if a block is smaller than seg_len
        ...
        len_keep = num_keep_per_sample.max().item()
        ...
        # 4. Build final binary mask (1 = should be reconstructed)
    ...
    def forward_encoder(self, x, mask_ratio): # x: [NM, T, V, C]
        ...
        self.valid_patch_mask = patch_mask.reshape(NM,  TP * VP)  # Flag the valid patch
        """
    ...
    def patchify(self, imgs): # Input: imgs: (N, T, V, 3)
        ...
        return x.reshape(shape=(NM, TP * VP, u * p * C))    # Output: x: (N, L, t_patch_size * patch_size * 3)
    ...
    def forward(self, x, mask_ratio=0.80, **kwargs):
        ...
        return loss, pred, mask # pred.shape, mask.shape: [B, TP * VP, t_patch_size * patch_size * 3] , [B, TP * VP]


    """
    # 1. completely random masking
    def random_masking(self, x, mask_ratio):
        ...
        ids_shuffle = torch.argsort(noise, dim=1)                   # sort noise for each sample, ascend - small is keep, large is remove/mask
        ids_restore = torch.argsort(ids_shuffle, dim=1)

        ids_keep = ids_shuffle[:, :len_keep]
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D)) # [B,len_keep, D]
        mask = torch.ones([N, L], device=x.device) 
        mask[:, :len_keep] = 0                            # 0 is keep input to encoder, 1 is remove/masked in input
    ...
    # 2. First mask entire frames may be more effective to learn temporal dynamics, then mask joints to learn spatial correlations
